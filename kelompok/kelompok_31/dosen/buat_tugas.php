...
document.getElementById('formTugas').addEventListener('submit', function(e){
    e.preventDefault(); fetch(API, {method:'POST', body:new FormData(this)}).then(r=>r.json()).then(d=>{ Swal.fire('Sukses','Tugas diterbitkan','success'); loadData(); this.reset(); });
});
function loadData(){
    fetch(API).then(r=>r.json()).then(res=>{
        ...
        res.data.forEach(i=>{
            h+=`<tr>
                ...
                <td><span class="text-danger small fw-bold">${new Date(i.deadline).toLocaleString()}</span></td>
